...
inline std::string create_model_acess_info_record(const std::string& mrenclave,
                                                  const std::string& user_id) {
...
bool load_and_unseal_data(const std::string& key, std::string* output,
  ...
  uint32_t unseal_buf_size = get_unsealed_data_size((const uint8_t*)value_buf);
  ...
  *status = unseal_data((const uint8_t*)value_buf,
                        static_cast<uint32_t>(retrieved_size), key.c_str(),
                        key.size(), unseal_buf, unseal_buf_size);
...
sgx_status_t seal_and_save_data(const std::string& key,
  ...
  int buf_size = get_sealed_data_size(static_cast<uint32_t>(key.size()),
                                      static_cast<uint32_t>(value.size()));
  uint8_t* seal_buf = (uint8_t*)malloc(buf_size);
  if (!seal_buf) return SGX_ERROR_OUT_OF_MEMORY;
  auto sgxStatus =
      seal_data(key.c_str(), static_cast<uint32_t>(key.size()), value.c_str(),
...
RequestExecCtx decrypt_payload(const std::string& user_id,
  ...
  // decrypt the buffer
  uint8_t* plain_txt;
  auto decode_bytes = base64_decode(
...
int send_client_reply(WOLFSSL* ssl, const RequestExecCtx& ctx) {
  auto reply =
      (ctx.sgxStatus != SGX_SUCCESS)
          ? KeyServiceReply(false, "Request failed: SGX error: " + ctx.msg)
                .EncodeTo()
          : ((ctx.request_failure)
                 ? KeyServiceReply(false, "Request failed: " + ctx.msg)
                       .EncodeTo()
...
sgx_status_t enc_worker_service(WOLFSSL* ssl, void* store) {
  ...
  if (!check_permission_in_access_list(
          ...
          create_model_acess_info_record(mrenclave, req.user_id()), store,
          &sgxStatus)) {
#ifndef NDEBUG
