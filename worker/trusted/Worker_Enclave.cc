...
SimpleCache<KeySet> key_cache{1};
// shared ra channel
...
int decode_decryption_keys(const std::string& ks_response) {
  ...
  int ret = DecodeKeyServiceWorkerReply(ks_response, &user_id, &input_key_b64,
                                        &model_id, &model_key_b64);

  // base64 decode the keys
  if (ret != 0) return ret;
  auto input_key_bytes =
      base64_decode((const uint8_t*)input_key_b64.data(), input_key_b64.size());
  auto model_key_bytes =
      base64_decode((const uint8_t*)model_key_b64.data(), model_key_b64.size());
...
sgx_status_t fetch_from_ks(const std::string& request,
  ...
  if (!channel) {
    channel =
        new RatlsChannelClient{ks_port, std::move(ks_addr), CheckContext{}};
...
// only allow single permitted model under equal_exec_time setting
  ...
  unsigned char* decrypt_input = NULL;
  {
    ...
    auto ret = decrypt_content_with_key_aes(
  ...
  ret = execute_inference((char*)decrypt_input,
                          get_aes_decrypted_size(input_data_size),
                          *decrypt_model, model_rt, prediction_buf,
                          OUTPUT_BUF_SZ, &prediction_size);
  ...
  *output_size =
      (ret == SGX_SUCCESS) ? get_aes_encrypted_size(OUTPUT_BUF_SZ) : 0;
